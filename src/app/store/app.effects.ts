...
import { ContactAction, SAVE_CONTACT, ADDED_CONTACT, UPDATED_CONTACT, LOAD_CONTACTS, LOADED_CONTACTS, REMOVE_CONTACT, REMOVED_CONTACT, LOAD_CONTACT, LOADED_CONTACT, SET_ERROR } from './actions/item.actions';

// Nice way to test error handling? localStorage.clear() after contacts are presented 
@Injectable()
    ...
    return this.actions$.pipe(
      ...
      switchMap((action) =>
        this.contactService.save(action.contact).pipe(
          ...
          map((savedItem) => ({
            type: (action.contact._id) ? UPDATED_CONTACT : ADDED_CONTACT,
            contact: savedItem,
          })),
          catchError((error) => {
            console.log('Effect: Caught error ===> Reducer', error)
